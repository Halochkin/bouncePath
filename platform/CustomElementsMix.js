...
function mixElement(className, mixins) {
  //1. typecheck mixins
  for (let mixin of mixins)
    if (!(mixin.prototype instanceof HTMLElement))  //todo: mixin.constructor instanceof Function ?
      throw new Error(
        `ElementMixins must be a direct descendant on HTMLElement, ie. "${mixin.name} extends HTMLElement".`);
  ...
  if (staticProps.observedAttributes) {
    ...
    if (observingMixins.length > 1) {
      const observedAttributes = observingMixins.map(mixin => mixin.observedAttributes).flat(1).filter(
        (e, i, a) => a.indexOf(e) === i);
  ...
  const MixedElement = class __mixed__ extends HTMLElement {
    constructor() {
      super();
    }
  }
