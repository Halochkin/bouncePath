...
const innerLinkSlot = outerHost.shadowRoot.querySelector("inner-link-slot");

// describe('Custom dispatchEvent ', function () {
//
//
//   it('basic, capture: true', function () {
//     let res = "";
...
//   })
//
//   it('basic, capture: false', function () {
//     let res = "";
//     for (let node of targets) {
//       node.addEventListener("my-event", function (e) {
...
describe("dsd", function () {

  it('stopPropagation', function () {
    let res = "";

    for (let node of targets) {
      let nodeNmae = nodeName(node)
      let cb = nodeNmae  === "##UP-IN-LI-SL" ? e => e.stopPropagation() : e => res += log(e, true, node);
      node.addEventListener("my-event", cb, true)
    }
    h1.dispatchEvent(new Event('my-event', {composed: true, bubbles: true}));
